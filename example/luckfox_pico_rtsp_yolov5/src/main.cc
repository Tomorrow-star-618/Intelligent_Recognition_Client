...
int main(int argc, char *argv[]) {
	...
	{	
		...
		if(s32Ret == RK_SUCCESS)
		{
            ...
            memcpy(rknn_app_ctx.input_mems[0]->virt_addr, letterboxImage.data, model_width*model_height*3); // 拷贝到模型输入
            inference_yolov5_model(&rknn_app_ctx, &od_results); // 推理
        ...
        memcpy(data, frame.data, width * height * 3); // 拷贝回编码内存
